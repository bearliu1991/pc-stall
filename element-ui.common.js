...
exports.default = {
  ...
  methods: {
    handleStart: function handleStart(rawFile) {
      ...
      this.tempArr = this.uploadFiles.slice()
      this.onChange(file, this.uploadFiles);
    },
    handleProgress: function handleProgress(ev, rawFile) {
      var file = this.getFile(rawFile);
      this.onProgress(ev, file, this.tempArr);
    ...
    handleSuccess: function handleSuccess(res, rawFile) {
      ...
      if (file) {
        ...
        this.onSuccess(res, file, this.tempArr);
        this.onChange(file, this.tempArr);
      }
    },
    handleError: function handleError(err, rawFile) {
      var file = this.getFile(rawFile);
      var fileList = this.tempArr;

      file.status = 'fail';

      fileList.splice(fileList.indexOf(file), 1);

      this.onError(err, file, this.tempArr);
      this.onChange(file, this.tempArr);
    },
    handleRemove: function handleRemove(file, raw) {
      ...
      if (!this.beforeRemove) {
        ...
        var before = this.beforeRemove(file, this.tempArr);
        if (before && before.then) {
          before.then(function () {
            doRemove();
          }, noop);
    ...
    getFile: function getFile(rawFile) {
      var fileList = this.tempArr; // this.uploadFiles;
      var target = void 0;
    ...
    submit: function submit() {
      ...
      this.tempArr.filter(function (file) {
  ...
  beforeDestroy: function beforeDestroy() {
    this.tempArr.forEach(function (file) {
